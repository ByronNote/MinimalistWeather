...
class GetCurrentWeatherUseCase @Inject constructor(
    ...
    suspend operator fun invoke(): DomainResult<Weather> {
        return when (val currentCityResult = settingsRepository.getCurrentCityId()) {
            is DomainResult.Success -> invoke(currentCityResult.data, refreshNow = false)
            is DomainResult.Failure -> currentCityResult
        }
    }

    suspend operator fun invoke(cityId: String?, refreshNow: Boolean): DomainResult<Weather> {
        ...
        return weatherRepository.getWeather(normalizedCityId, refreshNow)
    }
}
